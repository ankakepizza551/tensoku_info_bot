...
class ThreadIndexCog(commands.Cog):
    ...
    async def _sync_board(self, board: dict) -> None:
        ...
        try:
            ...
            return
        try:
            await message.edit(embed=_build_thread_index_embed(channel))
    ...
    @app_commands.command(
        ...
        description="このチャンネルのアクティブなスレッド一覧を自動更新するボードを設置します（管理者用）",
    )
    @app_commands.describe(channel="ボードを設置するチャンネル（省略時はこのチャンネル）")
    ...
    async def setup_thread_index(
        ...
        embed = _build_thread_index_embed(target)
        try:
        ...
        await db_manager.save_thread_index_board(
            channel_id=target.id, guild_id=interaction.guild_id, message_id=board_msg.id
        )
        await interaction.response.send_message(
            f"✅ {target.mention} にスレッド一覧ボードを設置しました。", ephemeral=True
        )
        logger.info(
            f"setup_thread_index: guild={interaction.guild_id} channel={target.id} message={board_msg.id}"
        )
    ...
    @app_commands.describe(channel="ボードを削除するチャンネル（省略時はこのチャンネル）")
    @app_commands.default_permissions(manage_guild=True)
    async def remove_thread_index(
        ...
        target = channel or interaction.channel
        board = await db_manager.get_thread_index_board(target.id)
        if board is None:
            await interaction.response.send_message(
                f"❌ {target.mention} にはボードが設置されていません。", ephemeral=True
            )
            return

        await db_manager.delete_thread_index_board(target.id)
        try:
        ...
        await interaction.response.send_message(
            f"✅ {target.mention} のスレッド一覧ボードを削除しました。", ephemeral=True
        )
        logger.info(f"remove_thread_index: guild={interaction.guild_id} channel={target.id}")
